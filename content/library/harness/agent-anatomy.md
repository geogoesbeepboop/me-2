...
sourceMtime: '2026-09-20T02:26:30.341Z'
...
contentHash: 'sha256:ef4bd08275442502616b49058a91c5293c328655145c4c4fe5620818c073e242'
---
...
**Do this.** Treat the context window as something you assemble per call and keep small, not
something that accumulates. Name six things: what loads always versus on demand; how the agent
retrieves just in time; what gets cleared; what is isolated in a subagent; the assembly order; and
who may write to anything that persists across runs.

| Mechanism | What it is | The decision you owe |
|---|---|---|
| Progressive disclosure | The always-loaded layer is an index — names, one-line descriptions, paths — and the detail loads when the task reaches it | What is always loaded, and why each line earns that |
| Skills | The on-demand loading primitive: a procedure, its references and scripts cost a description line until invoked | Which procedures are skills rather than system-prompt prose |
| Just-in-time retrieval | The agent holds lightweight identifiers (paths, queries, ids) and pulls data with tools at the moment of use, instead of pre-loading a retrieved corpus | Which data is pre-loaded (small, stable, always needed) and which is fetched |
| Tool-result clearing | Old tool calls and results are dropped once acted on — the lightest-touch form of compaction | What is cleared, after how many turns, and what is exempt |
| Compaction | Summarise and restart the window. A last resort, and a lossy one | The never-drop list: the outcome, the egress rules, the open side-effect record |
| Subagents as context isolation | A child burns its own window on search or review and returns a conclusion, not the file dumps | What a child returns, and its size cap |
| Cache-aware assembly order | Stable prefix first: tool definitions → system → durable facts → conversation → volatile retrieved content last | Order is a cost and latency decision — a change at any level invalidates every level after it |
| Auto-memory vs hand-curated files | The harness saves what it judges relevant; a person curates the instruction file | Who may write each persistent layer, and who reads it back before it loads |
| Subtractive ablation | Delete an instruction, rerun the evals, keep the cut if nothing regresses | The cadence, and the suite that judges it |

Four couplings with decisions already made. Clearing and compaction are loop transitions
(*Loop & state*) — what they drop is lost state. A compaction summary **and an auto-memory entry**
are memory writes (*Memory & retention*): each carries the trust of what it was derived from, not
the agent's. That makes auto-memory a **poisoning surface** — a hostile document summarised into a
memory is replayed into every later session as if the agent had concluded it, so auto-memory gets
the same least-trusted-writer rule, a review path, and a poisoning drill as any other store.
Placement is not authority — models do not reliably privilege earlier or system-slotted text over
later hostile text, so order buys cache hits and legibility, never enforcement; what must hold
regardless of the window belongs in deterministic gates (§1.1). And a subagent's return is
untrusted input to its parent (*Tools & child authority*).

**What it looks like.** A context policy in the agent's config: always-loaded = outcome, egress
rules, a skill index; refund procedure, vendor playbooks and the style guide are skills; order
history is fetched by id, never pre-loaded; tool results older than five turns are cleared, except
the open side-effect record; tool definitions are sorted deterministically and never edited
mid-session, so the cached prefix survives. Two evals: plant "never email vendor X" early, force
clearing and a compaction forty turns later, assert it still binds; and a quarterly ablation —
remove one instruction block, rerun the suite, keep the cut if the delta sits inside the noise
floor.
...
which is why nobody files it as a bug. The opposite failure is as common and quieter: a window
stuffed with instructions written for a weaker model, paid for on every call and obeyed too
literally by a stronger one.

**Evidence.** *Reported* — vendor engineering guidance, graded as such. Anthropic describes
just-in-time retrieval, progressive disclosure, tool-result clearing ("one of the safest lightest
touch forms of compaction"), structured note-taking and sub-agents with clean windows
([Anthropic, 2025-09-29](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents));
for the current model generation it reports removing over 80% of Claude Code's system prompt with
no measurable loss on its coding evaluations, and recommends deleting repeated instructions, moving
detail into skills, deferring tool definitions, and auto-memory over hand-saved notes
([Anthropic, 2026-07-24](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models)).
*Standard* for the ordering cost: cache prefixes build in the order tools → system → messages, and
a change at one level invalidates that level and all after it
([Anthropic, prompt caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)).
*House* for the couplings, the auto-memory poisoning rule, and ablation as a scheduled routine —
the vendor's 80% figure is one team's suite, not a law; your cut is only as good as the evals you
rerun. This decision previously prescribed fixed window slices and an 80%-fill compaction trigger;
both were unmeasured and are withdrawn.

**Wrong when.** A constraint that quietly fell out of the window gets read as a model failure
instead of an eviction bug — or the instruction file only ever grows.

**Yours to decide.** The always-loaded set, the clearing horizon, what compaction must never drop —
that list is the agent's constitution, and it should be short — and who reviews auto-memory writes.
...
**What it looks like.** A routing table, one line per route: "triage → the mid-tier
workhorse (suite green 2026-07-22); fallback → the small fast model, inherits the refund gate and the $5 tree ceiling, refund suite
...
**What it looks like.** A result line that stands alone: `pass 14/16 — model workhorse-model@2026-06-30 ·
prompt 7c3a · policy 12 · tools search-v3 · dataset 2026-07-22 · judge judge-model@cal-9 · env fp-a41
...
peanuts, ever"), the spend ceiling, the open-order record. The store catalogue is fetched by query,
never pre-loaded; substitution results are cleared once the item is settled; the retention eval
plants the allergy rule early, forces a compaction forty turns later, and asserts it still binds.
