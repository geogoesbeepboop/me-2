...
sourceMtime: '2026-08-03T01:26:46.000Z'
sourceCommit: bd4fcb7
syncedAt: '2026-08-03'
summary: >-
  Visual companion:
...
contentHash: 'sha256:1dec2a185b1a75366ebd3e0a59a6d23c70728e9d7f180c63d4fa537fd62f2f8a'
---
...
The **sixteen decisions** live inside those stages, keeping their canonical numbers (decision 7 is
decision 7 everywhere in this document). Decisions marked
***product-native*** (13–16) exist only because a product has something one agent never has: a peer
with its own owner, release cadence, and reliability target — the agent-scope checklist reaches
further than people expect (its T2/T3 fields already ask which contracts must stay stable and who
owns merge order), but it cannot ask any of that *across an ownership boundary*. Walk every stage
for a new product or a consequential change; jump straight to a decision for a refresher — each
one is written to stand alone.
...
citation ledger, enforced at commit time.
...
|---|---|---|---|---|
| 1 | Outcome | What single sellable result does the product own? | Shape | Demonstrated |
| 4 | Loop & topology | One agent or many — and who writes? | Shape | Demonstrated |
| 13 | Seam ownership *(product-native)* | Who owns what belongs to no agent? | Shape | House |
| 14 | Contracts *(product-native)* | How do independently-shipping agents keep agreeing — and what crosses the seam? | Shape | Demonstrated |
| 2 | Runtime authorization | Who may perform each irreversible action? | Constrain | Standard |
| 5 | Inputs & egress | What comes in, what leaves, and who enforces it? | Constrain | Standard |
| 6 | Memory | Who owns what the fleet remembers? | Constrain | House |
| 7 | Credentials & blast radius | What can the fleet touch when one agent falls? | Constrain | Standard |
| 8 | Human involvement | Where does a person actually decide? | Involve | Reported |
| 9 | Budgets that nest | What stops a fan-out from spending forever? | Meter | House |
| 15 | SLO composition *(product-native)* | What reliability does the product promise, and how do parts compose into it? | Meter | House |
| 10 | Observability | Can you reconstruct the outcome end to end? | Observe | Standard |
| 11 | Durability | What survives a kill −9? | Survive | Reported |
| 12 | Failure & compensation | Who undoes agent A's side effect? | Survive | House |
| 3 | Eval & version sets | What exactly did we just ship? | Prove | Demonstrated |
| 16 | Model routing *(product-native)* | Which model serves which role — and who checks the checker? | Ship | Demonstrated |
...
Before drawing a fleet, qualify it. The field's oldest advice here is also its best-evidenced:
...
orchestrator-workers on genuinely parallel research (decision 4). It is: **most published
...
### Decision 4 — loop, state, and topology
...
### Decision 13 — seam ownership *(product-native)*
...
### Decision 14 — cross-agent contracts, and what crosses them *(product-native)*
...
by the seam owner (decision 13). The payload half is measurable: a successor agent that receives a
...
as decision 13) — and **read that source against itself:** it concludes protocol-focused solutions
...
crosses carries the trust level of its writer (decision 6) and the information budget of its
...
internals ([A2A v1.0](https://a2a-protocol.org/latest/)) — which composes with decision 5's rule
...
### Decision 2 — runtime authorization
...
budget line, the requester's authority (decision 7), the daily ceiling, and it fails closed when
...
### Decision 7 — credentials and blast radius
...
B's spend gate (decision 2) evaluates the request against *A's* authority for that budget line —
deny, escalate to the queue (decision 8). The same request from the surface owner — allow. One
rule at one gate: *authority of the requester, for the resources named.* Without it, "A asks B"
produces an authorized call by B and the money moves — with decision 2 fully intact, because that
gate authorizes the identity that *calls* it. This is not only a read problem, and reads are the
first term of decision 5's lethal trifecta.
...
person directs, approves, and corrects the fleet. One decision owns it, and the design space is
wider than an approve button.

### Decision 8 — human involvement
...
**Beyond the queue.** Three surface patterns with real weight behind them, all *Reported* or
*Claimed* today and labeled accordingly. **Plan approval before execution** — the agent proposes,
a person approves the plan, the agent works read-only until then; shipped as protocol in agent
teams ([Claude Code](https://code.claude.com/docs/en/agent-teams)). **Transparency of planning
steps** — show the decomposition, not just the result, so a person can steer before tokens burn
([Anthropic, 2024-12-19](https://www.anthropic.com/engineering/building-effective-agents)).
**Steering mid-run** — interruption and redirection as first-class, not kill-and-restart
(*Claimed* — recurs across vendor surfaces, traced to no measured treatment). All three convert
human attention from re-verification (the expensive kind) to direction (the kind that scales).
...
person will wait, and what reliability the outcome promises. These are design inputs, not bills
you discover.

### Decision 9 — budgets that nest
...
### Decision 15 — product SLO composition *(product-native)*
...
reliability improves more slowly than capability across model generations — twelve metrics over
...
Decision 10 states the rule; the rest of this section is the model that implements it — rebuilt
...
*requested* model while a silent fallback served something else (decision 16).
...
└─ invoke_workflow brief_pipeline            gen_ai.workflow.name · gen_ai.conversation.id
   ...
   │  │  └─ chat claude-sonnet-5             gen_ai.request.model ≠ gen_ai.response.model → dec. 16
   ...
   ├─ gate egress_policy                     [H] gate.decision=allow · gate.rule_version → dec. 2
   ├─ approval await_publisher_signoff       [H] approval.wait_ms — decision 8's instrument
...
└─ invoke_agent publisher
   └─ execute_tool cms_publish               [H] idempotency.key → decision 12
...
1. **The approval pause is a span with a duration.** Its p50/p95 is decision 8's fatigue
...
class × **served** model, which makes decision 16's requested-vs-served field load-bearing for
...
Any agent can be killed mid-action. Machines restart, providers time out, sessions evaporate.
This stage decides what survives, who undoes what, and what the compute substrate owes the tasks
running on it.

### Decision 11 — durability
...
(decision 7's broker, made physical), and long async gaps — CI, review, a human's weekend — are
...
because of exactly the union blast radius decision 7 computes (same source).
...
### Decision 12 — failure and compensation
...
**What it looks like.** A compensation column in the seam register (decision 13): for each side
...
measure different things, two standing duties that cut across all of them, and one routing rule
...
### Decision 3 — eval and version sets
...
here: contract conformance (the handoff parses *and* carries what the schema promised — decision
14); graded handoff quality, not just validity; injection resistance *at the seam* (a hostile
payload in a peer's output — decision 5); cascade containment (fault-inject one agent, measure
what the blast radius actually was); termination under fan-out (decision 9's ledger holds under
...
escalation rate, **cost per accepted outcome** (§5), under decision 15's stated reliability target
...
state. These seven track **a change** moving toward production; decision 11's four track **a
...
position is engaged at decision 11.)
...
version or alias; keep the last known good route. Ship the whole thing as decision 3's immutable
...
### Decision 16 — model routing *(product-native)*
...
authority or budget**, the same non-amplification rule as decision 7, one altitude up.
...
- **Peer output trusted because it is ours** (§2, decision 5). *Standard.*
- **Credential union blast radius** — grants reasoned one agent at a time; delegation amplifies
  (§2, decision 7). *Standard* risk, *House* bound.
- **Approval fatigue as a control** — present on paper, rubber stamp in practice (§3, decision 8).
  *Claimed*, now measurable (§5).
- **Local compliance, global breach** — every child under its own ceiling, the parent breached
  (§4, decision 9). *Reported* failure, *House* fix.
...
- **The model as final allow/deny authority** (§2, decision 2). *Standard.*
...
**What this guide does not have evidence for.** Nested budget enforcement (decision 9) — no vendor
publishes a pattern. Cross-agent compensation ownership (decision 12). Agent-native
champion/challenger (§8). The autonomy-graduation thresholds (decision 8). The non-amplification
bound (decision 7) — RFC 8693 supplies the semantics and declines the ceiling. SLO composition
(decision 15) — propagation is measured, composition is not. The seam-eval category (§7) — the
grouping is ours even where its members are evidenced. Each says so in place.

**Decisions considered and left out.** From the original derive-then-diff pass: *commercial
pricing under nondeterministic unit cost* and *value-chain accountability* (bigger-N, not
product-native), *emergent collective behaviour* (measured, but no published mitigation — revisit
when a fix exists). From the 2026 research pass: *the organizational operating model* (review
economics at agent throughput, capacity planning — real, and a different document), *a
customer-facing autonomy taxonomy* (vocabulary, not a decision), *skills/procedural-knowledge
packaging* (harness-side; its ownership question folds into decision 6's store register), and *a
full human-surface pattern language* (sourcing too weak to carry a decision at this doc's
evidence bar — decision 8 carries the load-bearing subset, labeled *Claimed*).
...
**Dated sections.** §5's OTel status and decision 7's NIST guidance both rest on standards in
