...
sourceMtime: '2026-09-20T02:26:30.343Z'
...
contentHash: 'sha256:7e46f1ef88c731b4d8f01b200c5f40ce345678456fa02c5294e737e754d62bb3'
---
...
### MCP as a trust and transport boundary

Not a new decision — it is where *Inputs & egress* and *Credentials & blast radius* meet
when a tool lives behind someone else's server. An MCP server is two things at once: a
**transport** (how calls travel) and a **trust boundary** (whose text, whose UI and whose
credentials cross it). The 2026-07-28 spec revision changed the first a great deal and the second
not at all — everything a server returns is still untrusted input.

| 2026-07-28 change | What it is | What it means for this product |
|---|---|---|
| Stateless core | The `initialize` handshake and `Mcp-Session-Id` are gone; every request carries its protocol version and capabilities; any request can land on any instance. Cross-call state is an explicit server-minted handle passed as a tool argument | Servers scale like plain HTTP. But a handle is now a bearer value in the model's window — treat it like a credential: scoped, expiring, never the only check. Identity comes from the token, not the connection |
| Client ID Metadata Documents | Dynamic Client Registration is **deprecated** in favour of CIMD (it still works, for authorization servers without CIMD); clients must validate `iss`, and must key stored client credentials to their issuer | The client's identity is a URL the authorization server can fetch and you can allowlist, instead of anonymous self-registration. Per-agent client identity (above) gets a standard shape |
| Tasks as an extension | Experimental tasks left the core for `io.modelcontextprotocol/tasks`: poll with `tasks/get`, send input with `tasks/update` | A long-running call is a durable object with its own lifecycle — it needs a budget, a cancel path and an owner (*Durability*, *Budgets that nest*) like any other in-flight side effect |
| MCP Apps as an official extension | A tool may declare a `ui://` resource; the host renders server-supplied HTML in a sandboxed iframe that can request tool calls and update model context over `postMessage` | A **UI injection surface**. The sandbox protects the host page; it does not protect the *person* from a persuasive third-party UI, or the model from context the app pushes back. It is a human surface you did not write — apply *Human surfaces* and the egress list to it, and restrict which tools an app may call |

**Skills and MCP are complementary, not rivals.** MCP is *reach* — typed access to a system across
a trust boundary, with auth. A skill is *know-how* — a procedure loaded into context on demand, with
no boundary of its own. The usual pairing: an MCP server exposes the tools; a skill teaches when and
how to use them. The failure is using one for the other's job: a skill holding credentials, or a
server stuffing procedure prose into tool descriptions that load on every call.

**Permission systems need independent audit, not design confidence.** A spec-conformant OAuth flow
says who the client is; it does not say your allowlist is right, that a tool's scope matches its
description, or that a relayed approval is refused. Those are found by someone other than the
author trying to break them — a red-team pass over every server's tool list, scopes and UI
resources on connect and on change, with the results kept as eval cases (*Runtime authorization*).
"We designed it carefully" is a *Claimed* control.

**Evidence.** *Standard* for the protocol facts
...
[MCP Apps overview](https://modelcontextprotocol.io/extensions/apps/overview)). *House* for every
consequence drawn in the right-hand column, for the skills/MCP split, and for the audit rule.
...
└─ invoke_workflow brief_pipeline            gen_ai.workflow.name · gen_ai.conversation.id
   ...
   │  │  └─ chat <workhorse-model>           gen_ai.request.model ≠ gen_ai.response.model → Model routing
...
**What it looks like.** A release manifest: `planner v14 = {model: workhorse-model@2026-06-30, prompt:
7c3a, policy: 12, tools: [search v3, cart v9], eval-set: 2026-07-22, judge: judge-model@cal-9, env:
...
**Candidate roles, not names** — model names date faster than this file, so routes are written
against roles and the release manifest pins the exact id: a **frontier long-horizon model** for
planning and multi-hour work; a **mid-tier workhorse** for contracted execution; a **small fast
model** for triage, classification and fallbacks; a **cross-family judge** so a grader does not
share its subject's blind spots. Vendor positioning for any of them is *Reported* at best — never
promote without local task evidence.
...
**Build versus buy: managed agent platforms.** Several rows above now have a vendor-hosted
equivalent. Managed agent runtimes ship the loop, sandbox, compaction and caching; some add an
**outcome grader** — you supply a rubric, a grader in a separate context window scores the artifact
and the agent iterates to a cap — and vendor-run **memory curation** between sessions
([Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview), beta;
[outcomes](https://platform.claude.com/docs/en/managed-agents/define-outcomes); memory curation
("dreaming") is a limited research preview whose behaviour we have not read in detail) — *Reported*, vendor documentation.

| Buy when | Build (or keep) when |
|---|---|
| The harness is undifferentiated plumbing and you would otherwise spend the first month on a sandbox | The loop, the gate or the memory policy *is* the product |
| A rubric grader is good enough for the outcome | The outcome needs a programmatic verifier, a calibrated judge, or a committed calibration artifact — a vendor grader is still an LLM judge, so the grading rules in §7 apply to it unchanged |
| Session state may live with the vendor | Retention, deletion or residency rules say otherwise — check eligibility before designing around it |
| You can export traces, memory and evals | You cannot — then memory curated by a process you cannot inspect is a store with an unnamed writer (*Memory*) |

Buying the runtime never buys the decisions: outcome, authority, egress, budgets and the eval set
stay yours on either side of the line.
